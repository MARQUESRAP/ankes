...
import { ArrowLeft, Plus, Trash2, Save, Send, Download, Loader2, FileText, Eye, UserPlus } from 'lucide-react';
...
export default function NewInvoicePage() {
  ...
  const [nextNumber, setNextNumber] = useState('FAC-001');
  ...
  const [createFrom, setCreateFrom] = useState<'scratch' | 'quote'>('scratch');
  const [selectedQuoteId, setSelectedQuoteId] = useState('');
  const [clientId, setClientId] = useState('');
  const [issueDate, setIssueDate] = useState(formatDateForInput(new Date()));
  const [dueDate, setDueDate] = useState(formatDateForInput(new Date(Date.now() + 30 * 24 * 60 * 60 * 1000)));
  ...
  const loadData = async () => {
    try {
      const [clientsData, userData, number, quotesData] = await Promise.all([
        ...
        documentsService.getNextNumber('invoice'),
  ...
  const handleSelectQuote = async (quoteId: string) => {
    ...
    try {
      const quote = await documentsService.getById(quoteId);
      if (quote) {
  ...
  const quoteOptions = [
    { value: '', label: 'Sélectionner un devis' },
    ...quotes.map(q => ({ value: q.id, label: `${q.number} - ${q.client?.name || 'Client'} (${formatCurrency(q.total_ttc)})` }))
  ];
    ...
    <AppLayout>
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-800 dark:text-white">Nouvelle facture</h1>
            <p className="text-gray-500 dark:text-gray-400 mt-1">{nextNumber}</p>
          </div>
        </div>

        {/* Choix depuis devis ou from scratch */}
        <Card variant="default" padding="lg">
          <h2 className="text-lg font-bold text-gray-800 dark:text-white mb-4">Créer à partir de...</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <button
              onClick={() => { setCreateFrom('scratch'); setSelectedQuoteId(''); }}
            ...
            >
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="font-semibold text-gray-800 dark:text-white">Nouvelle facture</span>
              </div>
              <p className="text-sm text-gray-500 dark:text-gray-400">Créer une facture vierge</p>
            </button>
            <button
            ...
            >
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="font-semibold text-gray-800 dark:text-white">Depuis un devis</span>
              </div>
              <p className="text-sm text-gray-500 dark:text-gray-400">Convertir un devis accepté</p>
            </button>
          </div>

          {createFrom === 'quote' && (
            <div className="mt-4 animate-slideUp">
              <Select label="Devis à convertir" options={quoteOptions} value={selectedQuoteId} onChange={(e) => handleSelectQuote(e.target.value)} />
              {quotes.length === 0 && (
                <p className="text-sm text-amber-600 mt-2">Aucun devis accepté disponible. Acceptez d'abord un devis.</p>
              )}
            </div>
          )}
        </Card>
      ...
      {selectedClient && (
        <SendEmailModal
          ...
          templateSubject={user?.email_invoice_subject || emailService.getDefaultInvoiceTemplate().subject}
          templateBody={user?.email_invoice_body || emailService.getDefaultInvoiceTemplate().body}
          pdfBlob={pdfBlob}
        />
      )}
    </AppLayout>

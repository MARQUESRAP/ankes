...
export default function EditInvoicePage() {
  ...
  const handleSave = async (newStatus?: 'draft' | 'sent') => {
    ...
    try {
      ...
      if (newStatus) {
        await documentsService.updateStatus(invoiceId, newStatus);
      }
      ...
      if (newStatus === 'sent' && selectedClient) {
        ...
        const pdfUrl = await storageService.uploadPDF(blob, `${invoice?.number}.pdf`);
    ...
    return (
      <AppLayout>
        <div className="text-center py-12">
          ...
          <Button onClick={() => router.push('/invoices')} className="mt-4">Retour aux factures</Button>
        </div>
      </AppLayout>
    );
  }

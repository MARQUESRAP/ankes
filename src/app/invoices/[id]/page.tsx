...
export default function EditInvoicePage() {
  ...
  const loadData = async () => {
    try {
      const [clientsData, userData, invoiceData] = await Promise.all([
        ...
        documentsService.getById(invoiceId),
      ]);
      ...
      if (invoiceData) {
        ...
        setIssueDate(formatDateForInput(new Date(invoiceData.issue_date)));
        setDueDate(invoiceData.due_date ? formatDateForInput(new Date(invoiceData.due_date)) : '');
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      await documentsService.update(invoiceId, {
        client_id: clientId,
        issue_date: issueDate,
        due_date: dueDate,
        notes,
  ...
  const handleSaveAndSend = async () => {
    ...
    try {
      ...
      await documentsService.updateStatus(invoiceId, 'sent');
      ...
      if (selectedClient) {
        ...
        await storageService.uploadPDF(blob, `${invoice?.number}.pdf`);
      }
    ...
    <AppLayout>
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-800">Modifier la facture</h1>
          ...
          <div className="space-y-6">
            <Card variant="elevated" padding="lg" className="sticky top-8">
              ...
              <div className="space-y-3">
                ...
                <Button className="w-full" onClick={handleSaveAndSend} isLoading={isSaving} icon={<Send className="w-5 h-5" />}>
